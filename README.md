...
| Seeded demo conversations and caseload assignments | `backend/src/seed/seed.js` — the seed now assigns each demo student to Maria Delgado (ISS) and pre-populates a realistic multi-day message history for all three, so Messages, the dashboard's ISS contact card, and appointment booking aren't empty on a fresh install. Skipped once real messages/assignments exist, so it never overwrites a live conversation. |
