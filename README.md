...
| Student dashboard: visual urgency breakdown (overdue/due-soon/on-track), a month calendar of upcoming deadlines, a prioritized "needs attention" list, an ISS contact card with a message preview, and a recent-activity feed of staff reviews | `frontend/src/pages/StudentDashboardPage.jsx`, `frontend/src/components/DeadlineCalendar.jsx` — checklist and message data the dashboard already had access to, no new endpoints beyond `returned_at` on checklist items (for the activity feed) |
...
| Direct messaging between a student and their assigned ISS staff, with the contact's name/avatar shown up front | `backend/src/routes/messages.js`, `frontend/src/components/MessageThread.jsx` (shared by `MessagesPage.jsx` for students and embedded in `StudentDetailPage.jsx` for staff). One thread per student rather than per staff member, so coverage handoffs (FR9) don't lose history. Best-effort email notification to the other party on a new message. |
| Simulated ISS replies for demo purposes | `backend/src/simulatedReplies.js` — with no second live user to answer as staff during a walkthrough, a student message gets a topic-matched canned reply from their assigned ISS contact a few seconds later (with a "typing…" indicator in the UI). Every simulated message is flagged `is_simulated` and always labeled "Automated demo reply" — never passed off as a real response. Set `DEMO_AUTO_REPLIES=false` to turn this off (e.g. once a real staff member is actually answering). |
| Seeded demo conversations, caseload assignments, and checklist progress | `backend/src/seed/seed.js` — the seed now assigns each demo student to Maria Delgado (ISS), pre-populates a realistic multi-day message history for all three, and gives each checklist a genuine mix of approved/in-progress/returned/overdue/due-soon items instead of everything sitting at 0% not-started. So Messages, the dashboard (percent complete, urgency chart, calendar, needs-attention, recent activity), and appointment booking all look like a real in-progress case on a fresh install, not an empty shell. Skipped once real messages/assignments/checklist items exist, so it never overwrites live data. |
| Appointment scheduling: book a 30-minute chat with your ISS contact | `backend/src/routes/appointments.js`, `frontend/src/components/ScheduleAppointment.jsx` (under Messages). Availability is computed from fixed weekday business hours (9am–5pm, minus already-booked slots) — pick a day, then an open slot, and it's booked immediately; cancel any time before it happens. A unique index on `(staff_user_id, start_time)` for scheduled appointments prevents two students from ever double-booking the same slot, even under a race. Staff see a read-only list of upcoming appointments on the student's detail page. |
...
- **City Life content covers Seattle, WA only.** The data model (`backend/src/city/cityDocs.js`) is a flat curated set for one city; a multi-campus version would need this keyed by campus/city.
